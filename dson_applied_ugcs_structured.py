...
def looks_like_printable_string(raw_bytes):
    if not raw_bytes:
        return False
    return all(32 <= b <= 126 for b in raw_bytes)


def read_len_string_flexible(data, offset, max_len=200):
    """
    After a field name terminator, try:
      [pad zeros 0..4][u32 length][ascii bytes][optional null]
    """
    for pad in range(0, 5):
        ...
        if not looks_like_printable_string(raw_str):
            continue

        # Prefer candidates followed by a null terminator
        next_offset = str_end
        if next_offset < len(data) and data[next_offset] == 0:
            next_offset += 1
            return raw_str.decode("utf-8", errors="replace"), next_offset

        # Still allow it if the bytes look good
        return raw_str.decode("utf-8", errors="replace"), next_offset
